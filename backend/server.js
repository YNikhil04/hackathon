...
const questionBank = {
  jee: [
    {
      id: 1,
      question: "A particle moves in a circle of radius 5 cm with constant speed 10 cm/s. What is the acceleration of the particle?",
  ...
  gate: [
    ...
    {
      id: 2,
      question: "Which of the following is NOT a type of process scheduling algorithm?",
    ...
    {
      id: 3,
      question: "In a full binary tree with n leaves, the number of internal nodes is:",
...
app.get('/api/exam-types', (req, res) => {
  res.json({
    ...
    exams: [
      ...
      { id: 'gate', label: 'GATE', description: 'Graduate Aptitude Test in Engineering' },
    ],
  });
});
...
app.post('/api/start-test', (req, res) => {
  ...
  if (!questionBank[type]) {
    return res.status(404).json({ success: false, message: `Exam type "${examType}" not found` });
  }
...
app.post('/api/submit-test', (req, res) => {
  ...
  if (!examType || !answers) {
    return res.status(400).json({ success: false, message: 'examType and answers are required' });
  }
  ...
  if (!bank) {
    return res.status(404).json({ success: false, message: 'Invalid exam type' });
  }
